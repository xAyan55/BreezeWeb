...
const ScrollStack = ({
  ...
  // Optimization: Calculate offsets once and cache them
  const recalculateOffsets = useCallback(() => {
    ...
    if (useWindowScroll) {
      endElementTopRef.current = endElement ? endElement.getBoundingClientRect().top + window.scrollY : 0;
      cardOffsetsRef.current = cardsRef.current.map(card => card ? card.getBoundingClientRect().top + window.scrollY : 0);
    } else {
